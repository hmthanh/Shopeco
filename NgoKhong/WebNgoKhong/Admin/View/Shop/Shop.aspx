<%@ Page Language="C#" MasterPageFile="~/Admin/Layout/MasterPage.master" AutoEventWireup="true" CodeFile="Shop.aspx.cs" Inherits="Admin_View_SanPham_LoaiSanPham" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <style>
        .btnChecked, .btnUnChecked, .btnBlock, .btnUnBlock {
            cursor: pointer;
        }
         .btnChecked a, .btnUnChecked a, .btnBlock a , .btnUnBlock a {
            color:white !important;
        }
    </style>
    <%--<script src="<%= GetRouteUrl("jquery", new {}) %>"></script>--%>
    <script>
        $('.btnChecked').click(function () {
            if (!confirm("Bạn có muốn duyệt gian hàng không ?"))
            {
                return;
            }
            var shopid = $(this).data('idshop');
            var data = { action: 'Duyet', id: shopid };
            $.ajax({
                type: 'get',
        ...
        $('.btnUnChecked').click(function () {
            if (!confirm("Bạn có muốn hủy duyệt gian hàng không ?")) {
                return;
            }
            var shopid = $(this).data('idshop');
            var data = { action: 'HuyDuyet', id: shopid };
            $.ajax({
                type: 'get',
                url: location.origin + '/Ajax/Ajax_DonHang.aspx',
                dataType: 'JSON',
        ...
        $('.btnBlock').click(function () {
            if (!confirm("Bạn có muốn block gian hàng không ?")) {
                return;
            }
            var shopid = $(this).data('idshop');
            var data = { action: 'Block', id: shopid };
            $.ajax({
                type: 'get',
                url: location.origin + '/Ajax/Ajax_DonHang.aspx',
                dataType: 'JSON',
        ...
        $('.btnUnBlock').click(function () {
            if (!confirm("Bạn có muốn hủy block gian hàng không ?")) {
                return;
            }

            var shopid = $(this).data('idshop');
            var data = { action: 'HuyBlock', id: shopid };
            $.ajax({
                type: 'get',
                url: location.origin + '/Ajax/Ajax_DonHang.aspx',
                dataType: 'JSON',
