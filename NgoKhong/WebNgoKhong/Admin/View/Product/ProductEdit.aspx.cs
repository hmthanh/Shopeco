...
(--Convert delimited string to XML
 ...
 CROSS APPLY tempPlayer.nodes('/p/p') Members(Member)) as tb where idMau = m.idMau and idSanPham = '" + id + "') from tb_Mau m where KichHoat = 1";
        ...
        for (int i = 0; i < table.Rows.Count; i++)
        {
            ...
            html += "       <input class='uniform' onchange='CapNhatListMau(this.checked,\"" + table.Rows[i]["idMau"] + "\")' id='color_" + table.Rows[i]["idMau"] + "' " + (table.Rows[i]["sp"].ToString() != "" ? "checked" : "") + " type='checkbox'/>";
...
(--Convert delimited string to XML
 ...
 CROSS APPLY tempPlayer.nodes('/p/p') Members(Member)) as tb where idSize = m.idSize and idSanPham = '" + id + "') from tb_Size m where KichHoat = 1";
        ...
        {
            html += "   <span class='col-lg-3' style='display:flex'>";
            html += "       <input class='uniform' onchange='CapNhatListSize(this.checked,\"" + table.Rows[i]["idSize"] + "\")' id='size_" + table.Rows[i]["idSize"] + "' " + (table.Rows[i]["sp"].ToString() != "" ? "checked" : "") + " type='checkbox'/>";
    ...
    protected void btLuu_Click(object sender, EventArgs e)
    {
        ...
        {
            Response.Write("<script>alert('Bạn chưa chọn danh mục cấp 3!')</script>");
            return;
        }
        ...
        if (id == "0" || id == "")
        {
            ...
            string sqlInserNguoiDung = "insert into tb_SanPham(MaSanPham,TenSanPham,GiaCu,GiaMoi,Mau, Size,MoTa,idLoaiSanPham,idBrand,SetHome,SetHot,KichHoat, NgayTao)";
            sqlInserNguoiDung += " values(N'" + MaSanPham + "',N'" + TenSanPham + "',N'" + GiaCu + "',N'" + GiaMoi + "','" + Mau + "','" + Size + "',N'" + MoTa + "',N'" + idLoaiSanPham + "','" + idThuongHieu + "','" + TrangChu + "','" + TinHOT + "','" + KichHoat + "', '" + ngayTao + "')";
        ...
        else
        {
            ...
            sqlUpdateNguoiDung += " ,idBrand = N'" + idThuongHieu + "'";
            ...
            if (ktUpdateNguoiDung)
            {
                ...
                SaveHinh(id);
                if (Page != "")
                    Response.Redirect(GetRouteUrl("adminSanPham", new { page = Page}));
                Response.Redirect(GetRouteUrl("adminSanPham", new { }));
            }
            else
            {
