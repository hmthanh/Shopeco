...
public partial class Admin_View_TinTuc_LoaiTinTin_CapNhat : System.Web.UI.Page
{
    ...
    protected void btLuu_Click(object sender, EventArgs e)
    {
        ...
        if (id == "0" || id == "")
        {
            string sqlInserNguoiDung = "insert into tb_Banner(TieuDe,ThuTu, Url,Link,Active,isBanner)";
            sqlInserNguoiDung += " values(N'" + TieuDe + "',N'" + ThuTu + "', '" + Url + "','" + logo + "','" + KichHoat + "',0)";
            bool ktInsertNguoiDung = Connect.Exec(sqlInserNguoiDung);

            if (ktInsertNguoiDung)
            {
                Response.Redirect(GetRouteUrl("adminslide", new { }));
            }
            else
            {
        ...
        else
        {
            ...
            {
                if (Page != "")
                    Response.Redirect(GetRouteUrl("adminslide", new { page = Page}));
                Response.Redirect(GetRouteUrl("adminslide", new { }));
            }
            else
            {
