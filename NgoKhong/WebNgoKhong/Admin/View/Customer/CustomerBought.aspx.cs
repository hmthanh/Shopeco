...
public partial class Admin_View_SanPham_LoaiSanPham : System.Web.UI.Page
{
    ...
    private void loadData()
    {
        ...
        string sqlUserInfo = @"select * from tb_user d where d.ID = '" + idUser + "'";
        DataTable dataUserinfo = Connect.GetTable(sqlUserInfo);
        ...
        usCustomerCode.InnerHtml = dataUserinfo.Rows[0]["UserCode"].ToString();
        usTenNguoiDung.InnerHtml = dataUserinfo.Rows[0]["FullName"].ToString();
        usSoDienThoai.InnerHtml = dataUserinfo.Rows[0]["SoDienThoai"].ToString();
        usEmail.InnerHtml = dataUserinfo.Rows[0]["Email"].ToString();
        usDiaChi.InnerHtml = dataUserinfo.Rows[0]["Address"].ToString();




        //string status = Status[id - 1].ToString();
        string sql = @"select * from ( SELECT ROW_NUMBER() OVER ( ORDER BY d.idDonHang desc )AS RowNumber , *, 'TongTien' = (select isNull(Sum(SoLuong * DonGia),0) from tb_CTDonHang where idDonHang = d.idDonHang)  from tb_DonHang d where d.idUser = N'" + idUser + "'";
