    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                {
                    sIDUser = Request.QueryString["id"].Trim();
                    //slGianHang.Items.FindByValue(sIDUser).Selected = true;
                }
            }
            catch { }
            ...
            string sql = @"select * from ( SELECT ROW_NUMBER() OVER ( ORDER BY s.idSanPham desc )AS RowNumber
	              , s.idSanPham, g.TenGianHang, d.Ngay, s.MaSanPham, s.TenSanPham, s.GiaCu, s.GiaMoi, s.idLoaiSanPham from tb_CTDonHang c, tb_DonHang d, tb_SanPham s, tb_GianHang g where c.idDonHang = d.idDonHang and s.idSanPham = c.idSanPham and g.ID = s.IDGianHang and d.idUser = '" + id + "'";
