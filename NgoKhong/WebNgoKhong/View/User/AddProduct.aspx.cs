...
public partial class View_User_AddProduct : System.Web.UI.Page
{
    string id = "";
    //string idGianHang = "";
    string IDUser = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            IDUser = Session["CurrentUser"].ToString().Trim();
            string shopCode = StaticData.getField("tb_User", "ShopCode", "ID", IDUser);
            IDShop = StaticData.getField("tb_GianHang", "ID", "ShopCode", shopCode);
        }
        else
        {
            return;
    ...
    private void LoadThuongHieu()
    {
        ...
        slThuongHieu.DataTextField = "TenThuongHieu";
...
(--Convert delimited string to XML
 ...
 CROSS APPLY tempPlayer.nodes('/p/p') Members(Member)) as tb where idSize = m.idSize and idSanPham = '" + id + "') from tb_Size m where KichHoat = 1";
    ...
    {
        string sql = "select * from tb_LoaiDatHang";
        ...
        slLoaiDatHang.DataSource = data;
        slLoaiDatHang.DataTextField = "TenLoaiDatHang";
        slLoaiDatHang.DataValueField = "MaLoaiDatHang";
        slLoaiDatHang.DataBind();
        slLoaiDatHang.Items.Add(new ListItem("--Chọn loại đặt hàng--", ""));
        slLoaiDatHang.Items.FindByText("--Chọn loại đặt hàng--").Selected = true;
    }
    ...
    protected void btLuu_Click(object sender, EventArgs e)
    {
        ...
        string Mau = "0";
        string Size = "0";
        string maLoaiDatHang = "DATONLINE";
        ...
        {
            msgError.InnerHtml = "Bạn chưa nhập mã sản phẩm!";
        ...
        {
            msgError.InnerHtml = "Bạn chưa nhập tên sản phẩm!";
            msgError.Visible = true;
            return;
        }
        ...
        //    return;
        //}
        if (slLoaiDatHang.Value.Trim() != "")
        {
            maLoaiDatHang = slLoaiDatHang.Value.Trim();
        }
        if (slThuongHieu.Value.Trim() != "")
        {
            idBrand = slThuongHieu.Value.Trim();
        }
        else
        {
            msgError.InnerHtml = "Bạn chưa chọn thương hiệu";
            msgError.Visible = true;
            return;
        }
        if (idLoaiSanPham == "0")
        {
            msgError.InnerHtml = "Bạn chưa chọn loại sản phẩm !";
            msgError.Visible = true;
            return;
        }
        string hasImage = listImgHangHoa.InnerHtml.Trim();
        if (String.IsNullOrEmpty(hasImage))
        {
            msgError.InnerHtml = "Bạn chưa chọn hình ảnh !";
            msgError.Visible = true;
            return;
        }
        //Số Điện Thoại
        ...
        if (Session["CurrentUser"] == null)
        {
            ...
            Response.RedirectToRoute("userproduct", new { retUrl = url });
            return;
        }
        if (id == "0" || id == "")
        {
            string userID = Session["CurrentUser"].ToString();
            string shopCode = StaticData.getField("tb_User", "ShopCode", "ID", userID);
            string timeNow = StaticData.GetDate();
            string sqlInserNguoiDung = "insert into tb_SanPham(MaSanPham, TenSanPham, GiaCu, GiaMoi, Mau, Size, MoTa, idLoaiSanPham, idBrand, SetHome, SetHot, KichHoat, IDGianhang, MaLoaiDatHang, ShopCode, NgayTao)";
            sqlInserNguoiDung += " values(N'" + MaSanPham + "',N'" + TenSanPham + "',N'" + GiaCu + "',N'" + GiaMoi + "','" + Mau + "','" + Size + "',N'" + MoTa + "',N'" + idLoaiSanPham + "','" + idBrand + "','0','0','1', '" + IDShop + "', '" + maLoaiDatHang + "', '" + shopCode + "', '" + timeNow + "')";
            ...
            if (ktInsertNguoiDung)
            {
                ...
                string isDaDuyet = StaticData.getField("[tb_GianHang]", "DaDuyet", "ID", IDShop);
                if (isDaDuyet != "1")
                {
                    string sql = "update tb_SanPham set KichHoat = '0' where idSanPham = " + id;
