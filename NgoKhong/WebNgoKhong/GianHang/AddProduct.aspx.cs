...
(--Convert delimited string to XML
 ...
 CROSS APPLY tempPlayer.nodes('/p/p') Members(Member)) as tb where idSize = m.idSize and idSanPham = '" + id + "') from tb_Size m where KichHoat = 1";
    ...
    protected void btLuu_Click(object sender, EventArgs e)
    {
        ...
        if (id == "0" || id == "")
        {
            string sqlInserNguoiDung = "insert into tb_SanPham(MaSanPham, TenSanPham, GiaCu, GiaMoi, Mau, Size, MoTa, idLoaiSanPham, idBrand, SetHome, SetHot, KichHoat, IDGianhang, MaLoaiDatHang)";
            sqlInserNguoiDung += " values(N'" + MaSanPham + "',N'" + TenSanPham + "',N'" + GiaCu + "',N'" + GiaMoi + "','" + Mau + "','" + Size + "',N'" + MoTa + "',N'" + idLoaiSanPham + "','" + idBrand + "','0','0','1', '" + idGianHang + "', '" + maLoaiDatHang + "')";
            ...
            if (ktInsertNguoiDung)
            {
                ...
                idGianHang = Session["CurrentUser"].ToString();
                string isDaDuyet = StaticData.getField("[tb_GianHang]", "DaDuyet", "ID", idGianHang);
                if (isDaDuyet != "1")
                {
        ...
        else
        {
            ...
            sqlUpdateNguoiDung += " ,KichHoat = NULL";
