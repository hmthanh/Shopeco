...
public partial class View_GianHang_ThongTinGianHang : System.Web.UI.Page
{
    ...
    private string GetSQL_ListAll()
    {
        ...
        sqlPrefix = @"select * from ( SELECT ROW_NUMBER() OVER ( ORDER BY " + orderby + " )AS RowNumber ,[idSanPham], [TenSanPham], [GiaMoi], 'LinkHinh' = (select top 1 LinkHinh from tb_HinhAnhSanPham where idSanPham = s.idSanPham) from tb_SanPham s, tb_GianHang g where s.IDGianhang = g.ID and s.IDGianHang = " + shopID + " ";
