...
public partial class Admin_View_TinTuc_LoaiTinTuc : System.Web.UI.Page
{
    ...
    private void LoadLoaiTinTuc()
    {
        string sql = @"select * from
            (
	            SELECT ROW_NUMBER() OVER
	              ...
	              ,*
                  FROM tb_Banner nd where isBanner = 1";
        if (sTieuDe != "")
        ...
        for (int i = 0; i < table.Rows.Count; i++)
        {
            ...
            html += "       <td style='text-align:center;vertical-align: inherit;'><a href='" + GetRouteUrl("adminbannerupdate", new { id = idBanner}) + "' title='Sửa'><i class='fa fa-pencil'></i></a></td>";
