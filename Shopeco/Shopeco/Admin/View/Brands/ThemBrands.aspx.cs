...
public partial class Admin_View_Brands_ThemBrands : System.Web.UI.Page
{
    string id = "";
    ...
    private void LoadThongTinLoaiTinTuc()
    {
        if (id != "")
        {
            string sql = "select * from tb_Brands where idBrand ='" + id + "'";
            DataTable table = Connect.GetTable(sql);
            if (table.Rows.Count > 0)
            {
                btLuu.Text = "SỬA";
                txtTenThuongHieu.Value = table.Rows[0]["TenThuongHieu"].ToString();
    ...
    protected void btLuu_Click(object sender, EventArgs e)
    {
        ...
        if ((imageUploadInput.PostedFile != null) && (imageUploadInput.PostedFile.ContentLength > 0))
        {
            ...
            string SaveLocation = Server.MapPath("/images/Brands/") + fn;
            try
            {
                imageUploadInput.PostedFile.SaveAs(SaveLocation);
                //Response.Write("The file has been uploaded.");
                logo = "/images/Brands/" + fn;
            }
            catch (Exception ex)
            {
        ...
        //Tên Người Dùng
        TenThuongHieu = txtTenThuongHieu.Value.Trim();
        //Số Điện Thoại
        ...
        if (id == "0" || id == "")
        {
            string sqlInserNguoiDung = "insert into tb_Brands(TenThuongHieu,ThuTu,Link,Active,isBrand)";
            sqlInserNguoiDung += " values(N'" + TenThuongHieu + "',N'" + ThuTu + "','" + logo + "','" + KichHoat + "',0)";
            bool ktInsertNguoiDung = Connect.Exec(sqlInserNguoiDung);

            if (ktInsertNguoiDung)
            {
                Response.Redirect(GetRouteUrl("adminbrands", new { }));
            }
            else
            {
        ...
        else
        {
            ...
            sqlUpdateNguoiDung += "update tb_Brands set";
            sqlUpdateNguoiDung += " TenThuongHieu = N'" + TenThuongHieu + "'";
            sqlUpdateNguoiDung += " ,ThuTu = N'" + ThuTu + "'";
            if (logo != "")
                sqlUpdateNguoiDung += " ,Link = '" + logo + "'";
            sqlUpdateNguoiDung += " ,Active = '" + KichHoat + "'";
            sqlUpdateNguoiDung += " where idBrand ='" + id + "'";
            bool ktUpdateNguoiDung = Connect.Exec(sqlUpdateNguoiDung);
            if (ktUpdateNguoiDung)
            {
                if (Page != "")
                    Response.Redirect(GetRouteUrl("adminbrands", new { page = Page }));
                Response.Redirect(GetRouteUrl("adminbrands", new { }));
            }
            else
            {
