...
public partial class View_Home_Index : BasePage
{
    ...
    private void LoadInfo()
    {
        ...
        for (int i = 0; i < data.Rows.Count; i++)
        {
            ...
            string col2 = data.Rows[i]["Title"].ToString();
            string typeID = data.Rows[i]["TypeID"].ToString();

            string col3 = Utils.getField("PageType", "Name", "ID", typeID);

            
            html += "<tr>";
            html += "        <td>" + col1 + "</td>";
            html += "        <td><a href='" + GetRouteUrl("editpage", new { id = data.Rows[i]["ID"].ToString() }) + "'>" + col2 + "</a></td>";
            html += "        <td>" + col3 + "</td>";
            html += "</tr>";
        }
